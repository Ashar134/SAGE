...
function ApplicationPage() {
  ...
  const [selectedApplicationId, setSelectedApplicationId] = useState<string | null>(
    dummyApplications.length ? dummyApplications[0].id : null
  );
  ...
  const getProgressStep = (application: Application): number => {
    // 1 = Applied, 2 = Test, 3 = Interview, 4 = Final Decision
      ...
      <div className="application-container">
        ...
        {applications.length === 0 ? (
          ...
          <div className="applications-layout">
            {/* Left side: list of applications (clickable cards) */}
            <div className="applications-list">
              {applications.map((application) => (
                <div
                  key={application.id}
                  className={`application-card ${selectedApplicationId === application.id ? 'selected' : ''}`}
            ...
            {/* Right side: details for selected application */}
            <div className="application-detail-panel">
              {selectedApplication ? (
                <>
                  ...
                  <div className="application-details">
                    <div className="detail-item">
                      ...
                      <span className="detail-value">{selectedApplication.jobTitle}</span>
                    </div>
                    <div className="detail-item">
                    ...
                    <div className="detail-item">
                      ...
                      <span className="detail-value">{selectedApplication.location}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Application Date:</span>
                      <span className="detail-value">{formatDate(selectedApplication.applicationDate)}</span>
                    </div>
                    ...
                    <div className="assessment-section">
                      <div className="assessment-header">
                        ...
                        {selectedApplication.testStatus === 'passed' && (
                          <span className="test-passed-badge">✓ Passed</span>
                        )}
                        {selectedApplication.testStatus === 'failed' && (
                          <span className="test-failed-badge">✗ Failed</span>
                        )}
                        {selectedApplication.testStatus === 'completed' && (
                          <span className="test-completed-badge">Completed</span>
                        )}
                      </div>
                      {hasAttemptedTest(selectedApplication) ? (
                        <div className="detail-item">
                          <span className="detail-label">Test Status:</span>
                          <span className="detail-value">
                            {selectedApplication.testStatus === 'passed' && '✓ Passed'}
                            {selectedApplication.testStatus === 'failed' && '✗ Failed'}
                            {selectedApplication.testStatus === 'completed' && 'Completed'}
                          </span>
                        </div>
                      ) : (
                        <div className="detail-item">
                          ...
                          {(() => {
                            const testDaysRemaining = getDaysRemaining(selectedApplication.testDeadline);
                            return (
                              <span className={`detail-value ${testDaysRemaining < 0 ? 'deadline-passed' : testDaysRemaining <= 3 ? 'deadline-soon' : ''}`}>
                                {formatDate(selectedApplication.testDeadline)}
                                {testDaysRemaining >= 0 && (
                      ...
                      {canTakeTest(selectedApplication) && (
                        <button
                          className="action-button test-button"
                          onClick={() => handleGiveTest(selectedApplication.id)}
                        >
                          {selectedApplication.testStatus === 'in_progress' ? 'Continue Test' : 'Give Test'}
                        </button>
                      )}
                    </div>

                    {/* Interview Section - Only show if test is passed */}
                    {selectedApplication.testStatus === 'passed' && selectedApplication.interviewDeadline && (
                      <div className="assessment-section interview-section">
                        <div className="assessment-header">
                          <h3 className="assessment-title">Interview</h3>
                          {selectedApplication.interviewStatus === 'completed' && (
                        ...
                        {hasAttemptedInterview(selectedApplication) ? (
                          <div className="detail-item">
                            <span className="detail-label">Interview Status:</span>
                            <span className="detail-value">
                              {selectedApplication.interviewStatus === 'completed' && 'Completed'}
                              {selectedApplication.interviewStatus === 'passed' && '✓ Passed'}
                              {selectedApplication.interviewStatus === 'failed' && '✗ Failed'}
                            </span>
                          </div>
                        ) : (
                          <div className="detail-item">
                            ...
                            {(() => {
                              const interviewDaysRemaining = selectedApplication.interviewDeadline
                                ? getDaysRemaining(selectedApplication.interviewDeadline)
                                : null;
                              return (
                                <span className={`detail-value ${interviewDaysRemaining !== null && interviewDaysRemaining < 0 ? 'deadline-passed' : interviewDaysRemaining !== null && interviewDaysRemaining <= 2 ? 'deadline-soon' : ''}`}>
                                  {formatDate(selectedApplication.interviewDeadline)}
                        ...
                        {canAttendInterview(selectedApplication) && (
                          <button
                            className="action-button interview-button"
                            onClick={() => handleAttendInterview(selectedApplication.id)}
                          >
                            Attend Interview
                          </button>
                        )}
                ...
                <div className="empty-detail">
                  ...
                  <p>Click on a job on the left to view its details and progress.</p>
                </div>
              )}
            </div>
          </div>
