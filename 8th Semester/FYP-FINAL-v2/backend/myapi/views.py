...
def parse_cv(request):
    ...
    try:
        ...
        if application_id:
            ...
            # Try to attach to latest application without a resume; otherwise create a placeholder record
            latest_app = (
            ...
            if latest_app:
                ...
                # Create a lightweight application just to bind the resume
                app_obj = Application.objects.create(
                    user_id=user_id,
                    job_title="Resume Upload",
                    company_name="Resume",
                    company_logo_color="#6366f1",
                    status="reviewing",
                    resume_url=resume_url,
                )
        # Also store on user profile if we have a user_id
        if user_id:
            try:
...
def _serialize_hr_applicant(app):
    ...
    return {
        ...
        'match_score': None,
        'video_url': user.interview_recording_url if user and user.interview_recording_url else resume_url,
        'resume_url': resume_url,
...
def hr_applicants(request):
    ...
    try:
        qs = Application.objects.select_related('user').prefetch_related('user__education','user__skills').all()
