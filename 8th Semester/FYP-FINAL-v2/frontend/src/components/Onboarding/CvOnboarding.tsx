...
const CvOnboarding = () => {
  ...
  const { user, accessToken, login, isAuthenticated } = useAuth();
  ...
  useEffect(() => {
    if (profileDraft) {
      localStorage.setItem(
        "onboardingDraft",
        JSON.stringify({ __userId: user?.id || null, ...profileDraft })
      );
    }
  }, [profileDraft, user?.id]);

  // If user already has a profile, prefill the draft so editing doesn't force CV upload again
  useEffect(() => {
    const loadDraftForUser = () => {
      ...
      try {
        ...
        if (parsed.__userId && user?.id && parsed.__userId === user.id) {
          // strip helper key
          const { __userId, ...rest } = parsed;
          setProfileDraft(rest as ProfileDraft);
        } else if (parsed.__userId && user?.id && parsed.__userId !== user.id) {
    ...
    loadDraftForUser();
  }, [hasDraft, user?.id, accessToken, isAuthenticated]);
      ...
      <div className="cv-onboarding-content-wrapper">
        ...
        {!hasDraft ? (
          prefillChecked ? (
            ...
            <div className="upload-section-container">
              <div className="validation-message info" style={{ textAlign: "center" }}>
                <div className="spinner small" />
