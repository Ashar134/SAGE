...
export default function Dashboard() {
  ...
  const filteredLineData = useMemo(() => {
    ...
    const entries = filteredApplicants
      .map((a) => {
        const dateOnly = toDateOnly(a.appliedDate);
        if (!dateOnly) return null;
        return { date: dateOnly, applications: 1 };
      })
      .filter(Boolean);
  ...
  const barData = useMemo(() => {
    ...
    const sorted = Object.entries(counts)
      .map(([department, applicants]) => ({ department, applicants }))
      .sort((a, b) => b.applicants - a.applicants);
    return sorted.slice(0, 6); // top departments only
  }, [filteredApplicants]);

  const exportApplicants = () => {
    const rows = filteredApplicants.length ? filteredApplicants : applicants;
    if (!rows.length) {
    ...
    const header = ["Name", "Email", "Role", "Department", "Status", "Applied Date", "Candidate Code"];
    const lines = rows.map((row) => [
      ...
      escapeCsv(row.candidateCode || row.id),
    ].join(","));
    ...
    link.href = url;
    link.download = `applicants-${Date.now()}.csv`;
    link.click();
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-md bg-gray-900 text-white px-3 py-2 text-sm font-semibold hover:bg-gray-800"
          >
            <Download size={16} />
            Download
          </button>
      ...
      <Card className="shadow-sm border-gray-200">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          {/* Actions button removed per request */}
        </CardHeader>
        <CardContent>
